...
int proc_alloc() {
  ...
  for (uint i = 1; i <= MAX_NPROCESS; i++)
    if (proc_set[i].status == PROC_UNUSED) {
      ...
      proc_set[curr_pid].creation_time = mtime_get();
...
void proc_free(int pid) {
  ...
  ulonglong current_ts = mtime_get();
  int turnaround_time_ms = (current_ts - proc_set[pid].creation_time) / 1000;

  INFO("Turnaround time %d ms Total number of interrupts: %d\n", turnaround_time_ms, proc_set[pid].total_interrupts);
  if (pid != GPID_ALL) {
    earth->mmu_free(pid);
